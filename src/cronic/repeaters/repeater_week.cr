module Cronic
  class RepeaterWeek < Repeater #:nodoc:
    ...
    def this(pointer = :future)
      ...
      case pointer
      when :future
        this_week_start = Cronic.time_class.local(@now.year, @now.month, @now.day, @now.hour) + RepeaterHour::HOUR_SECONDS
        first_week_day_repeater = RepeaterDayName.new(@repeater_day_name)
        first_week_day_repeater.start = @now
      ...
      when :past
        this_week_end = Cronic.time_class.local(@now.year, @now.month, @now.day, @now.hour)
        first_week_day_repeater = RepeaterDayName.new(@repeater_day_name)
        first_week_day_repeater.start = @now
        last_span = first_week_day_repeater.next(:past)
