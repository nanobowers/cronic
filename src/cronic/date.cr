module Cronic
  class Date
    ...
    def self.wd_diff(date, modifier = 1)
      diff = modifier
      wday = date.wday
      if modifier == 0 && wday == DAYS[:sunday]
        diff = 1
      elsif modifier == 0 && wday == DAYS[:saturday]
        diff = 2
      elsif modifier == 1 && wday == DAYS[:friday]
        diff = 3
      elsif modifier == 1 && wday == DAYS[:saturday]
        diff = 2
      elsif modifier == -1 && wday == DAYS[:sunday]
        diff = -2
      elsif modifier == -1 && wday == DAYS[:monday]
        diff = -3
      end
      diff
    end
